...
<div class="container-fluid bg-white my-3">
    ...
    <h3 align="start">Chat With Parents</h3><br />
			<br />
			<div class="row">
...
$(document).ready(function () {
	...
	function make_chat_dialog_box(recipient_id, to_user_name) {
		var modal_content = '<div id="user_dialog_' + recipient_id + '" class="user_dialog" title="You have a chat with ' + to_user_name + '">';
		modal_content += '<div style="height:300px; border:1px solid #ccc; overflow-y: scroll; margin-bottom:24px; padding:16px;" class="chat_history" data-touserid="' + recipient_id + '" id="chat_history_' + recipient_id + '">';
		modal_content += fetch_user_chat_history(recipient_id);
		modal_content += '</div>';
		modal_content += '<div class="form-group">';
		modal_content += '<textarea name="chat_message_' + recipient_id + '" id="chat_message_' + recipient_id + '" class="form-control chat_message"></textarea>';
		modal_content += '</div><div class="form-group" align="right">';
		modal_content += '<button type="button" name="send_chat" id="' + recipient_id + '" class="btn btn-info send_chat">Send</button></div></div>';
